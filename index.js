document.addEventListener("DOMContentLoaded", () => {
    ...
    // Sembunyikan halaman landing setelah 2 detik dan tampilkan konten utama
    setTimeout(() => {
      landingPage.style.display = "none";
      mainContent.style.display = "block";
    }, 2000);
  
     // Fungsi untuk menangani klik pada tombol Create Jobsheet
    createJobsheetButton.addEventListener("click", () => {
      // Tampilkan modal password seperti biasa
      passwordModal.style.display = "flex";
    });
  
    // Ketika tombol submit pada modal diklik, verifikasi password dan arahkan ke halaman jobsheet jika benar
    submitPasswordButton.addEventListener("click", () => {
      const enteredPassword = passwordInput.value;
      const errorImage = document.getElementById("error-image");
  
      if (enteredPassword === "pitukuberkah123") {
        // Password benar, sembunyikan modal dan arahkan ke halaman jobsheet
        passwordModal.style.display = "none";
        window.location.href = "jobsheet.html"; // Arahkan ke halaman jobsheet
      } else {
        // Password salah, tampilkan gambar error
        errorImage.style.display = "flex";
  
        // Sembunyikan gambar setelah beberapa detik (opsional)
        setTimeout(() => {
    ...
    // Event listeners untuk pencarian limbah
  ...
  async function search() {
    ...
    if (input.trim() === "") {
      ...
      // Menyembunyikan tombol dan kemudian menampilkannya kembali dengan gaya di tengah
      createJobsheetButton.style.display = "block";
      createJobsheetButton.style.margin = "0 auto";
  
      // Sembunyikan kontainer limbah yang dipilih ketika input pencarian kosong
      selectedWastes = []; // Kosongkan array selectedWastes
      updateSelectedWasteContainer(); // Perbarui tampilan selectedWasteContainer
    ...
    try {
      ...
      if (wasteData.length === 0) {
        createJobsheetButton.style.display = "block";
        createJobsheetButton.style.margin = "0 auto"; // Pastikan tombol tetap di tengah
      }
    } catch (error) {
  ...
  function displayResults(data) {
    const results = document.getElementById("results");
    results.innerHTML = "<h3>Daftar Limbah</h3>";
